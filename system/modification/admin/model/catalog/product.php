...
class ModelCatalogProduct extends Model {
    ...
    $this->db->query("DELETE FROM " . DB_PREFIX . "ocfilter_option_value_to_product WHERE product_id = '" . (int)$product_id . "'");
		...
		foreach ($data['product_description'] as $language_id => $value) {
			...
			$value['meta_description'] = 'Интернет - магазин мебели Meblaks предлагает купить '.$value['name'].' > Низкая цена, гарантия, отзывы, доставка в Киев, Черкассы, Харьков, Одессу, Днепр, Украине';
			$value['meta_keyword'] = $value['name'].' купить, '.$value['name'].' цена, '.$value['name'].' заказать';
