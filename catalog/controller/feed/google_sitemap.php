...
class ControllerFeedGoogleSitemap extends Controller {
	public function index() {
		if ($this->config->get('google_sitemap_status')) {
			...
			$output .= '<url>';
					$output .= '<loc>' . $this->url->link('common/home', '', true) . '</loc>';
		...
		//			$output .= '</image:image>'; -->
			...
			foreach ($manufacturers as $manufacturer) {
				...
				foreach ($products as $product) {
					$output .= '<url>';
					$output .= '<loc>' . $this->url->link('product/product', 'manufacturer_id=' . $manufacturer['manufacturer_id'] . '&product_id=' . $product['product_id'], true) . '</loc>';
					$output .= '<changefreq>weekly</changefreq>';
					$output .= '<priority>0.7</priority>';
					$output .= '</url>';
				}
			...
			$blogs = $this->model_blog_blog->getSitemap();

			foreach ($blogs as $blog) {
				$output .= '<url>';
				$output .= '<loc>' . $this->url->link('blog/blog', 'blog_id=' . $blog['blog_id'], true) . '</loc>';
				$output .= '<changefreq>weekly</changefreq>';
				$output .= '<priority>0.7</priority>';
				$output .= '</url>';
			}
	...
	protected function getProducts($parent_id, $current_path = '') {
		$output = '';
